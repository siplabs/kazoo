...
-export([make_request/3, make_response/2, make_response/3]).
...
make_response(Realm, Req) ->
    make_response(Realm, Req, nksip_lib:luid()).

-spec make_response(binary(), nksip:request(), binary()) ->
    binary().

make_response(Realm, Req, Nonce) ->
    #sipmsg{
        app_id = AppId, 
        call_id = CallId,
...
check_auth_header(AuthHeader, Resp, User, Realm, Pass, Req) ->
    ...
    of
        ...
        false ->
            ...
            QOP = nksip_lib:get_value(auth, AuthHeader, []),
