...
handle_req(JObj, _Props) ->
    lager:info("handle_nonce ~p",[JObj]),
    'true' = wapi_gsm:nonce_req_v(JObj),
    _ = wh_util:put_callid(JObj),
    Username = get_auth_user(JObj),
    Realm = wapi_authn:get_auth_realm(JObj),
    lager:debug("trying to nonce ~s@~s", [Username, Realm]),
...
check_nonce(JObj, <<"IMSI", _R/binary>>,  'undefined', KeyHex, Response) ->
    ...
    SIMSResBin = imsi_util:hexstr_to_bin(SIMSRes),
    SIMSResHex = imsi_util:bin_to_hexstr(SIMSRes),
    Key = imsi_util:hexstr_to_bin(KeyHex),
    Nonce = imsi_util:hexstr_to_bin(NonceHex),
    SRes = imsi_crypto:a3a8(Nonce, Key),
    SResHex = wh_util:to_lower_binary(imsi_util:bin_to_hexstr(SRes)),
...
send_auth_resp(#auth_user{password=Password
    ...
    Resp = props:filter_undefined(
             [{<<"Msg-ID">>, wh_json:get_value(<<"Msg-ID">>, JObj)}
              ...
              | wh_api:default_headers(Category, <<"nonce_resp">>, ?APP_NAME, ?APP_VERSION)
             ]),
    lager:info("NONCE sending SIP authentication reply, with credentials ~p",[Resp]),
    wapi_gsm:publish_nonce_resp(wh_json:get_value(<<"Server-ID">>, JObj), Resp).
