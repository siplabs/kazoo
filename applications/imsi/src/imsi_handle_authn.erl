...
handle_req(JObj, _Props) ->
    lager:info("imsi_handle_authn ~p",[JObj]),
    'true' = wapi_gsm:authn_req_v(JObj),
...
send_auth_resp(#auth_user{password=Password
    ...
    lager:info("IMSI sending SIP authentication reply, with credentials ~p",[Resp]),
    wapi_gsm:publish_authn_resp(wh_json:get_value(<<"Server-ID">>, JObj), Resp).
