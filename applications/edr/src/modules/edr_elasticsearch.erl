...
init(#backend{options = ConnectionInfo})->
    Options = props:filter_undefined([{'connect_timeout', wh_json:get_integer_value(<<"Connect-Timeout">>, ConnectionInfo)}
                                      ,{'response_timeout', wh_json:get_integer_value(<<"Response-Timeout">>, ConnectionInfo)}
                                      ,{'is_ssl', wh_json:is_true(<<"Is-SSL">>, ConnectionInfo)}
                                      ,{'stream_to', self()}
                                      ,{'max_sessions', wh_json:get_integer_value(<<"Max-Sessions">>, ConnectionInfo)}
                                     ]),
    case wh_json:get_value(<<"Url">>, ConnectionInfo) of
...
-spec push(state(), non_neg_integer(), wh_json:object()) -> work_result().
push(#state{options = Opts, url = Url}, Timestamp, Data)->
    RawBody = wh_json:from_list([{<<"Timestamp">>, Timestamp}
                                 ,{<<"Edr">>, Data}
                                ]),
    Body  = wh_json:encode(RawBody),
    Headers = [],                               %TODO?
