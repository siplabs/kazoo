...
-spec start_backend(ne_binary(), ne_binary(), wh_json:object(), wh_json:object()) -> sup_startchild_ret().
start_backend(Name)->
    Backends = whapps_config:get(<<"edr">>, <<"backends">>, wh_json:new()),
    case wh_json:get_value(Name, Backends) of
        'undefined' -> {'error', 'not_registred'};
        JBackend ->
            Type=wh_json:get_value(<<"Type">>, JBackend),
            JTags=wh_json:get_value(<<"Tags">>, JBackend),
            JOpts=wh_json:get_value(<<"Options">>, JBackend),
            start_backend(Name, Type, JTags, JOpts)
    end.
start_backend(Name, Type, Tags, Opts)->
    Module = wh_util:to_atom("edr_" ++ binary_to_list(Type)),
