...
-callback(push(ModuleState :: any(), Timestamp :: non_neg_integer(), EventData :: wh_json:object()) -> work_result()).
...
-spec push(pid(), non_neg_integer(), wh_json:object())-> 'ok'.
...
%% Internal functions
%%------------------------------------------------------------------
-spec is_correspond(wh_json:object(), wh_json:object()) -> boolean().
is_correspond(JReceivedTags, JExpectedTags)->
    case wh_json:is_empty(JExpectedTags) of
        'true' -> 'true';
        _False -> ReceivedTags = wh_json:to_proplist(JReceivedTags),
                  ExpectedTags = wh_json:to_proplist(JExpectedTags),
