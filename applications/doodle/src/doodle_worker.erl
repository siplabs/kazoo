...
do_deliver_sms([], Props, JobId, JObj) ->
 'ok';                                       
do_deliver_sms([ Contact | Contacts], Props, JobId, JObj) ->
    ...
    wh_amqp_worker:cast('whapps_amqp_pool'
                       , Payload
                       ,fun wapi_sms:publish_forward/1).
    ...
    case get_number(Number) of
        {'ok', Num } ->
            ...
            case lookup_callflow(Number, AccountDb) of
                ...
                {'error', E } ->
                    lager:info("no callflow"),
                    deliver_offnet(Number, JobId, JObj)
            end;
        {'error', _E } ->
...
deliver_offnet(Number, JobId, JObj) ->
    lager:info("deliver offline not ready yet"),
    {'failed', <<"deliver offline not ready yet">>, JObj}.
