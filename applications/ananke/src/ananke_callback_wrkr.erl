...
start_link(Req, Attempts, Interval)
  when is_integer(Attempts) andalso Attempts > 0
    ...
    gen_server:start_link(?MODULE, [Req, Attempts, Interval * ?MILLISECONDS_IN_SECOND], []).

-spec init(#state{}) -> {'ok', #state{}, integer()}.
init([Req, Attempts, Interval]) ->
...
-spec handle_info(any(), #state{}) -> {'noreply', #state{}, integer()} | {'stop', any(), #state{}}.
